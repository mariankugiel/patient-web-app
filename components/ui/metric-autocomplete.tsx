...
interface MetricAutocompleteProps {
  value: string
  onChange: (value: string, selectedTemplate?: MetricTemplate | null) => void
  ...
  isLoading?: boolean
  disabled?: boolean
  onNewMetric?: (metricName: string) => void
}

export function MetricAutocomplete({
  value,
  onChange,
  templates,
  ...
  disabled = false,
  onNewMetric,
}: MetricAutocompleteProps) {
  const [isOpen, setIsOpen] = useState(false)
  const [filteredTemplates, setFilteredTemplates] = useState<MetricTemplate[]>([])
  ...
  useEffect(() => {
    ...
    const exactMatch = templates.find(
      t => t.display_name.toLowerCase() === query || t.name.toLowerCase() === query
    )
  ...
  const handleCreateNew = () => {
    if (value.trim() && onNewMetric) {
      onNewMetric(value.trim())
      setIsOpen(false)
      setSelectedIndex(-1)
    }
  }
    ...
    <div className={cn("relative", className)}>
      <div className="relative">
        <Input
          ...
          placeholder={disabled ? "Select a section first..." : placeholder}
          className={cn(
            "border-2 pr-10",
      ...
      {/* Dropdown Results */}
      {!disabled && isOpen && (filteredTemplates.length > 0 || showNewOption || isLoading) && (
        <div
          ref={listRef}
